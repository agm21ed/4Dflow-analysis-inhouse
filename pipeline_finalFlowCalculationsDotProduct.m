function pipeline_finalFlowCalculations3(iSubject)

disp(['Processing subject HV00' num2str(iSubject)]);
...
resultsdir  = (['/DSTORE/BRICIA/amorgan_PhD/4DFlowProject/4Dv2Danalysis/' char(Subjectdata{iSubject,1}) '/FlowResults_DotProduct']);%/' vessels '/']);
...
for iFlowScan = 1:13
    %% load data
    flowDataDir  = (['/DSTORE/BRICIA/amorgan_PhD/4DFlowProject/4Dv2Danalysis/' char(Subjectdata{iSubject,1}) '/FlowData_DotProduct/' vesselNames{iFlowScan}]);
    ...
    disp(['Processing ' vesselNames{iFlowScan}]);
    ...
    %% Peak velocity
    peakVel(iFlowScan) = (max(FD1.dotVectorMatrixCorr,[],'all'))/10; % finds maximum velocity across all pixels and timeframes for this vessel
    ...
    variableTable(variableTable==0) = NaN; % replace zeros with NaNs
...
sgtitle(['Subject' Subjectdata{iSubject,1} ' with dot product analysis']);
